...
node dist/backend/serverAdmin.js\
      ...
      --backend ${CONFIG_BASEDIR}backend.${CONFIGPROFILE}.json\
      --renameFileIfExists true\
      --srcFile ${PDOC_BASEDIR}staticdata/static.mysqhpdocs.js \
      --file ${PDOCS_TMP_BASEDIR}static.mysqhpdocs-${PROFILE}-${LANG}.json \
      --debug 1
node dist/backend/serverAdmin.js\
      --command convertPDoc\
      --action migrateLegacyPDocFile\
      ...
      --backend ${CONFIG_BASEDIR}backend.${CONFIGPROFILE}.json\
      --renameFileIfExists true\
      --srcFile  ${PDOCS_TMP_BASEDIR}static.mysqhpdocs-${PROFILE}-${LANG}.json \
      --langkeys lang_de,lang_en \
      --profiles ${PROFILES} \
      --file  ${PDOCS_TMP_BASEDIR}static.mysqhpdocs-${PROFILE}-${LANG}.json \
      --debug 1
node dist/backend/serverAdmin.js\
      ...
      --renameFileIfExists true\
      --srcFile ${PDOCS_TMP_BASEDIR}static.mysqhpdocs-${PROFILE}-${LANG}.json \
      --file ${PDOC_BASEDIR}staticdata/static.mysqhpdocs.js \
      --exportId assets/staticdata/static.mysqhpdocs.js \
      --debug 1
```
...
node dist/backend/serverAdmin.js\
      ...
      --renameFileIfExists true\
      --srcFile ${PDOCS_TMP_BASEDIR}static.mysqhpdocs-${PROFILE}-${LANG}.json \
      --file ${PDOCS_TMP_BASEDIR}static.mysqhpdocs-${PROFILE}-${LANG}-import.json\
      --debug 1
node dist/backend/serverAdmin.js\
      --command loadPDoc\
      --action loadDocs\
